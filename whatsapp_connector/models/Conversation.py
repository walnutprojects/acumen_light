...
class AcruxChatConversation(models.Model):
    ...
    def new_message(self, data):
        ...
        if send_bus:
            ...
            self.env['bus.bus']._sendone(conversation.get_bus_channel(),
    ...
    def update_conversation_bus(self):
        ...
        self.env['bus.bus']._sendone(self.get_channel_to_many(),
    ...
    def new_message_event(self, connector_id, msgid, data):
        ...
        if message_id:
            ...
            if data['type'] == 'failed':
                self.env['bus.bus']._sendone(conv_id.get_bus_channel(),
                                             'error_messages', data_to_send)
            else:
                self.env['bus.bus']._sendone(conv_id.get_channel_to_many(),
                                             'update_conversation', data_to_send)
        return message_id
    ...
    def search_conversation_by_partner_domain(self, partner_id):
        return [('res_partner_id', '=', partner_id),
                ('connector_id.company_id', '=', self.env.user.company_id.id)]
    ...
    def conversation_verify(self):
        ...
        for conn_id in Connector.search([]):
            ...
            if len(all_ids):
                ...
                notifications.append((all_ids[0].get_channel_to_many(), 'new_messages', to_news.build_dict(22)))
        if notifications:
    ...
    def block_conversation(self):
        ...
        if self.status in ['new', 'done']:
            ...
            if back_status == 'new':
                self.env['bus.bus']._sendone(channel, 'delete_conversation', [data_to_send])
        else:
    ...
    def init_and_notify(self):
        ...
        channel = self.get_channel_to_one()
        self.env['bus.bus']._sendone(channel, 'init_conversation', data_to_send)
