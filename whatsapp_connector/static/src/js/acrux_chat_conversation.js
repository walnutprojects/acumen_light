...
var Conversation = Widget.extend({
    ...
    init: function(parent, options) {
        ...
        this.context = {
            message_from_chatroom: true
        }
        _.extend(this.context, this.options.context);
        this.count_new_msg = 0;
        this.session = session;
        
        this.update(this.options);
        // el campo markAsSeen porque es un cmapo ficticio de js
        this.markAsSeen = this.options.markAsSeen
        ...
        if (messages && messages instanceof Array) {
            if (messages.length > 0) {
                if (messages[0] instanceof Message) {
                    ...
                    this.messages = [];
                    this.addClientMessage(messages);
                }
            } else {
                this.messages = [];
            }
    ...
    addClientMessage: function(messages) {
        ...
        if (messages) {
            ...
            if (show && messages.length) {
                ...
                if (!document.hidden) {
                    this.messageSeen(); // marca mensaje como visto
                }
            }
            messages.forEach(r => {
    ...
    createMessage: function(options) {
        ...
        let json_data = msg.export_to_json();
    ...
    messageSeen: function() {
        if (this.count_new_msg > 0 || !this.markAsSeen) {
            this._rpc({
                model: 'acrux.chat.conversation',
