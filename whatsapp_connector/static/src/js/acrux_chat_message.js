...
var Message = Widget.extend({
    ...
    willStart: function() {
        ...
        if (this.res_model && !this.res_model_obj) {
            ...
            if (this.res_model == 'product.product') {
                field_read = this.parent.product_used_fields
                if (this.parent.parent.product_used_fields) {  // por los mensajes citados
                    field_read = this.parent.parent.product_used_fields
                } else {
                    field_read = this.parent.parent.parent.product_used_fields
                }
            } else {
                field_read = ['id', 'checksum', 'mimetype', 'display_name', 'url']
            }
            def = this._rpc({
                ...
                if (result.length > 0) {
                    ...
                    if (this.res_model == 'product.product') {
                        ...
                        if (this.parent.parent.format_monetary) {
                            /** @eye esto solo acepta una anidacion de mensajes */
                            price = this.parent.parent.format_monetary(price);
                        } else {
                            price = this.parent.parent.parent.format_monetary(price);
                        }
    ...
    makeMessageDragAndDrop: function() {
        if (this.isToDrop()) {
            this.$el.draggable({
                revert: true,
                revertDuration: 0,
                containment: this.parent.parent.$el,
                appendTo: this.parent.parent.$el,
                helper: 'clone',
            });
        }
    },
     ...
     */
    getDate: function() {
        return field_utils.format.date(this.date_message, {}, {timezone: true});
    },
    ...
    audioController: function() {
        ...
        this.$audio = this.$player.prev();
        this.audio_obj = this.$audio[0];
        ...
        let calculateTime = (num) => {
            ...
            let hours = Math.floor(minutes / 60.0), out = '';
        ...
        this.$audio.on('loadeddata', _event => {
            // cargó todo el audio se muestra su duración
            this.$player.removeClass('o_hidden');
            this.$time.html(calculateTime(this.audio_obj.duration));
            resolve();
        });
        this.$audio.on('error', _event => {
            // si hubo un error cargando el audio
        ...
        this.$audio.on('timeupdate', event => {
            // se cambió la posición de resprodución del audio
    ...
    createAttachComponent: function(attachment) {
        ...
        if (!attachment.attachmentLists || attachment.attachmentLists.length === 0) {
            const attch_list = AttachmentList.insert(vals)
            attachment.update({
                isAcrux: true,
                attachmentLists: link(attch_list),
                isParentConversation: !(this.parent instanceof Message),
            })
        } else {
            const attch_list = AttachmentList.get(attachment.attachmentLists[0].localId)
            attch_list.update(vals)
            attachment.update({isParentConversation: !(this.parent instanceof Message)})
        }
