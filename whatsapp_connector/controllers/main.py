...
class WebhookController(http.Controller):
    ...
    def acrux_webhook(self, connector_uuid, **post):
        ...
        try:
            ...
            Connector = request.env['acrux.chat.connector'].with_user(SUPERUSER_ID)
            ...
            Conversation = request.env['acrux.chat.conversation'].with_user(SUPERUSER_ID)
            Conversation = Conversation.with_context(ctx)
    ...
    def mail_attachment_upload(self, ufile, thread_id, thread_model, is_pending=False, **kwargs):
        ...
        try:
            ...
            if len(datas) > 2000000:
                raise UserError(_('Too big, max. %s (%s)') % ('2 Mb', ufile.filename))
            vals = {
                'name': ufile.filename,
                'raw': datas,
                'res_id': 0,
