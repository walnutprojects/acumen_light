...
var AcruxChatAction = AbstractAction.extend({
    ...
    init: function(parent, action, options) {
        ...
        this.defaultChannelID = this.options.active_id ||
            ...
            'acrux_chat_live_id';
        let widget_options = { context: this.action.context };
    ...
    getDefaultAnswers: function() {
        return this._rpc({
            ...
            method: 'search_read',
            args: [[['show_in_chatroom', '=', true]], ['name', 'sequence', 'id', 'text', 'ttype', 'res_model', 'res_id']]
        }).then(result => {
    ...
    /**
     * Consulta los campos que se utilizan siempre en los productos
     *
     * @returns {Promise}
     */
    ...
    changeStatusView: function() {
        ...
        this.conversations = [];
        if (this.user_status.isActive()) {
            this.getServerConversation().then(() => this.showConversations());
        }
    ...
    showDefaultAnswers: function() {
        ...
        let target = this.$('div.default_table_answers');
        let padding = this.default_answers.length % row_size;
    ...
    onNewMessage: function(d) {
        ...
        if (conv) {
            ...
            def_out = conv.addClientMessage(d.messages);
    ...
    onInitConversation: function(conv_data) {
        let out;
        if (conv_data.agent_id[0] == session.uid) {
            out = this.addInitConversation(conv_data)
        } else {
            out = Promise.resolve();
        }
        return out;
    ...
    /**
     * Agrega una conversaciones desde una operaciones de iniciar
     *
     * @param {Object} conv_data Conversación a iniciar.
     * @returns {Pomise}
     */
    addInitConversation: function(conv_data) {
        ...
        return def.then(() => this.showConversationPanel())
            .then(() => this.selectConversation({ currentTarget: conv.el }))
            .then(() => conv);
    },
